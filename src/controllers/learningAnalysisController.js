...
const student = async (req,res,next)=>{try{let examIds=null;if(req.user.role==='teacher'){examIds=await service.teacherExamIds(req.user.id)}res.json(success(await service.analyze(Number(req.params.userId), examIds)))}catch(e){next(e)}};
